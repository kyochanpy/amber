use amber_core::{Compactor, Storage};
...
pub struct CompactSummary {
    pub created_parquet_files: usize,
    pub compacted_segments: usize,
    pub deleted_segments: usize,
}

pub async fn run_compact(args: &CompactArgs) -> Result<CompactSummary> {
...
mod tests {
    use amber_core::{
        AmberConfig, CatalogEvent, CatalogState, FoldedWalSegmentState, ObjectPath, SessionId,
        SessionManifest, WalWriteRequest, WalWriter,
    };
    use tempfile::TempDir;

    use super::*;
    use crate::test_support::{metadata_enriched_batch, write_config};
    ...
    async fn compact_command_compacts_closed_segments_without_cleanup() {
        ...
        assert!(
            storage
                .exists(&ObjectPath::from(compacted_segment_path))
                .await
    ...
    async fn compact_command_can_cleanup_compacted_segments() {
        ...
        for segment in state.wal_segments.values() {
            assert!(
                !storage
                    .exists(&ObjectPath::from(segment.path.clone()))
                    .await
